...
SQL alone does not always say which table an unqualified column belongs to. When you pass a dbplyr lazy table, dplyneage automatically lists the columns of each referenced table from the live connection and hands that schema to sqlglot, so attribution is exact.
...
- **First call**: may take a moment while the Python environment initializes (and, on the very first run, provisions)
