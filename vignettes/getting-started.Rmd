...
dplyr/dbplyr pipelines are analyzed entirely in R, so for those there is nothing else to install. Raw SQL strings are analyzed by sqlglot, dplyneage's one Python dependency — declared via `reticulate::py_require()` and provisioned automatically the first time it's needed, with no setup step. See `vignette("python-integration")` if you manage your own Python environment.
