...
In `lineage_flow()`, hovering a labeled column shows a card with the label and type. `lineage_json()` records them as per-node `labels` and `types` maps, and each OpenLineage schema-facet field gains a `description` — the [OpenLineage article](https://tgerke.github.io/dplyneage/articles/openlineage.html) shows the rendered event.
