...
silver feeds gold. Pass `extract_lineage()` a named list, one element
per layer, and it stitches them into a single DAG: any source table
whose name matches another element’s name links to that model’s node.
...
descriptions (from `label` attributes, database comments, or a `labels`
argument), and `events = "job"` produces the spec’s run-less design-time
events, one per model. `lineage_emit()` POSTs any of it to a running
backend:
