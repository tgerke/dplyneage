...
using `dbplyr::sql_render()` - Parses the SQL with sqlglot to extract
column-level lineage - Automatically creates nodes and edges for
visualization - Supports multiple SQL dialects (DuckDB, PostgreSQL,
MySQL, Snowflake, BigQuery, etc.)
...
- `inst/htmlwidgets/lineage_flow.yaml` - Widget dependencies
  configuration
- `srcjs/` - React Flow bundle source code (requires build with
  `./build_bundle.sh`)
...
- `extract_lineage(sql, dialect)` - Extract column lineage from SQL
  query or dbplyr lazy table
- `install_sqlglot(method, envname)` - Install Python sqlglot dependency
- `has_sqlglot()` - Check if sqlglot is available

**Bundle Management:**

- `build_bundle(force)` - Build the React Flow JavaScript bundle
  (requires Node.js)
