...
#' carrying the `columnLineage` facet. `events = "dataset"` emits
...
#' [extract_lineage()] captures that URI from the table's live
#' connection: `duckdb:<path>` and `sqlite:<path>` for file-backed
#' databases (bare `duckdb`/`sqlite` in memory, since the spec names no
#' convention for either), the spec's host:port form for server
#' databases. Each dataset in the event uses the namespace captured for
#' its model. A
